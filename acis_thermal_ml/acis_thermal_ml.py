...
from .utils import time2000, secs_per_year, data_map
import pandas as pd
import numpy as np
...
class ACISThermalML(object):
    ...
    def get_all_data(self, data, inputs):
        # This is the contribution from the variation in the solar
        # heating due to the Earth's elliptical orbit
        t_year = (data[self.msid].times - time2000) / secs_per_year
        states = self.get_cmd_states(data.datestart, data.datestop, data[self.msid].times)
        combined_dict = {'msid_times': data[self.msid].times,
                         'msid_vals': data[self.msid].vals,
                         'phase': np.cos(2*np.pi*t_year)}
        for input in inputs:
